...
def run():
    ...
    filename = os.path.join(args.output_directory, 'pertask_%s.arff' % args.classifier_name)
    ...
    with open(filename, 'w') as fp:
        arff.dump(openmlcontrib.meta.dataframe_to_arff(setup_data_with_meta_features,
                                                       relation_name,
                                                       json.dumps(json_meta)), fp)
    print(sklearnbot.utils.get_time(), 'Stored ARFF file with results to', filename)
